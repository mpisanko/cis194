...
localMaxima x@(_:y:z) = concatMap (\(a,b,c) -> bool [] [b] (b > a && b > c)) $ zip3 x (y : z) z
localMaxima _ = []
-- localMaxima = catMaybes . unfoldr (case splitAt 3 of
--                                           ((a:b:[c]),t) -> bool (Just (Nothing, t)) (Just (Just b,t)) (b > a && b > c)
--                                           _ -> Nothing)
...
-- hm = map (fromMaybe 0 . (flip lookup) $ (map (head &&& length)) . group . sort) [0..9]
